...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (900 * this.itemScale * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.width * (700 * this.itemScale * this.scale)) / this.screen.width;
...
class App {
  constructor(
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend, textColor, borderRadius, font, showTitles, itemScale, itemGap);
    this.update();
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font, showTitles, itemScale, itemGap) {
    const defaultItems = [
  ...
  onTouchMove(e) {
    ...
    this.didDrag ||= Math.abs(this.start - x) > 6;
  ...
  onClick() {
    ...
    const activeMedia = this.medias.reduce((closest, media) => (
      Math.abs(media.plane.position.x) < Math.abs(closest.plane.position.x) ? media : closest
    ));

    this.onItemSelect(activeMedia.item);
  }
  onWheel(e) {
  ...
  addEventListeners() {
    ...
    if (typeof IntersectionObserver !== 'undefined') {
      this.visibilityObserver = new IntersectionObserver(this.boundOnIntersection, { threshold: 0.01 });
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    let app;
    let isMounted = true;
    const resolvedFont = showTitles ? resolveFont(font, fontUrl) : Promise.resolve(font);

    resolvedFont.then(loadedFont => {
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, showTitles, itemScale, itemGap, scrollSpeed, scrollEase, onItemSelect]);
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular recent works gallery. Drag or scroll to browse. Press Enter to view the centered work."
    />
  );
}
