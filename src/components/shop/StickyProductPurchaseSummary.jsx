...
const StickyProductPurchaseSummary = ({ product, imageUrl, formatPrice, onAddToCart, onCustomOrder }) => {
  ...
  const actions = (
    <>
      {canPurchase && (
        <Button onClick={onAddToCart} className="h-11 flex-1 rounded-xl bg-[#47003C] text-white hover:bg-[#5A124E]">
          Add to cart
        </Button>
      )}
      <Button onClick={onCustomOrder} variant={canPurchase ? 'outline' : 'default'} className="h-11 flex-1 rounded-xl border-[#88538C] text-[#47003C] hover:bg-[#F7F0FA]">
    ...
    <>
      ...
      >
        <div className="flex gap-3">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden bg-[#E7DED3] text-[#47003C]">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-lg font-bold tabular-nums text-[#47003C]">{formatPrice(product.price)}</p>
      ...
      >
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <p className="shrink-0 font-bold tabular-nums text-[#47003C]">{formatPrice(product.price)}</p>
        </div>
